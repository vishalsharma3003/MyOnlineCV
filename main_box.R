...
main_box <- function(cv_mode) {
  if (cv_mode == "full") {
    tabBox(
      ...
      tabPanel(
        title = HTML(paste0(icon("graduation-cap"), "Education")),
      ...
      tabPanel(
        title = HTML(paste0(icon("university"), "Courses")),
    ...
    tabBox(
      ...
      tabPanel(
        title = HTML(paste0(icon("microphone"), "Experience")),
        uiOutput("talk_timeline")
      ),
      tabPanel(
        title = HTML(paste0(icon("archive"), "Projects")),
        # wrap in fluidRow so that elements
        # are added by column
        fluidRow(uiOutput("experience"))
      ),
